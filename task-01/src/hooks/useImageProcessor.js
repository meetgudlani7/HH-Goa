    ...
    async (file) => {
      ...
      try {
        ...
        if (!isLikelyImage && !hasImageExtension) {
          ...
          console.warn(
            'File does not have image MIME type or extension, checking magic bytes:',
            file.name
          );
        }
        ...
        if (isHeic) {
          ...
          // If the browser can convert HEIC with heic-to, do so.
          if (isWasmSupported()) {
            try {
              ...
              console.warn(
                'HEIC conversion failed, trying native browser support if available:',
                heicError
              );
              ...
              try {
                ...
                // The file itself is very likely fine — this usually means the photo
                // uses a newer HEIC encoding (e.g. recent iPhone + iOS 18+) that this
                // browser's converter and Chrome's own lack of native HEIC support can
                // both fail to decode. Point the user at an actionable workaround
                // instead of implying the file is broken.
                throw new Error(
                  "This HEIC photo couldn't be converted — it may use a newer format this browser doesn't support yet. Try exporting it as JPG/PNG (on iPhone: Settings → Camera → Formats → “Most Compatible”), or upload from a different device."
                );
              }
            }
          } else {
            ...
            try {
              ...
              URL.revokeObjectURL(originalObjectURL);
              throw new Error(
                'HEIC conversion is not supported in this browser. Please upload a JPG or PNG file.'
              );
            }
          }
        } else {
